...

{
  ...
  "plan": "P（計画）：本日行った施術内容、次回の目安、生活指導"
    ...
    : `あなたは整体院・カイロプラクティック院の優秀なカルテ作成アシスタントです。施術中の会話の文字起こしから、以下7項目のカルテをJSON形式のみで生成してください。会話に該当情報がない項目は空文字("")にしてください。JSON以外の文字は絶対に出力しないでください。

{
  ...
  try {
    ...
    res.json({ karte: parsed, format: is_soap ? "soap" : "standard" });
  } catch (err) {
...
app.post("/api/visits", async (req, res) => {
  ...
  try {
    const { data: patient } = await supabase
      .from("patients")
      .select("name, visits(id, date, karte, created_at)")
      .eq("id", patient_id)
      .single();
